...
Deno.test("StoredTx roundtrip - multiple outputs including opreturn", () => {
	...
	tx.vout = [
		...
		{ value: 0n, spentBy: null, scriptPubKey: { kind: "opreturn", value: new Uint8Array([0x6a, 0x04, 0x01, 0x02, 0x03, 0x04]) } },
	];
	const [decoded] = StoredTx.decode(StoredTx.encode(tx));
...
Deno.test("StoredTx encodeWithOffsets - vout offsets count matches vout length", () => {
	const tx = makeStoredTx();
...
Deno.test("StoredTx encodeWithOffsets - offsets are within bounds and strictly increasing", () => {
	const tx = makeStoredTx();
	const { bytes, offsets } = StoredTx.encodeWithOffsets(tx);
