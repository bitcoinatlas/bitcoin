 ...
 */
export class OldFixedKVStore<K, V> {
	...
	private async flush(): Promise<void> {
		...
		for (let i = 0; i < entries.length; i++) {
			...
			if (
				blockBufferPos + this.keyCodec.stride + this.valueCodec.stride > this.blockSize ||
				i === entries.length - 1
			) {
		...
		while (left <= right) {
			...
			if (cmp === 0) {
				return data.subarray(
					offset + this.keyCodec.stride,
					offset + this.keyCodec.stride + this.valueCodec.stride,
				);
			} else if (cmp < 0) {
				left = mid + 1;
			} else {
